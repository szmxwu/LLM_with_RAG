...
# 部位知识图谱
bodypartsknowledgegraph = pd.read_excel(os.path.join(_DICTIONARIES_DIR, "报告助手部位词典.xlsx"), sheet_name=0)
knowledgegraph = []
...
# 标题知识图谱
titlePartsKnowledgegraph = pd.read_excel(os.path.join(_DICTIONARIES_DIR, "报告助手部位词典.xlsx"), sheet_name=1)
...
# 报告词汇清洗
_replace_path = os.path.join(_DICTIONARIES_DIR, 'replace.xlsx')
ReplaceTable = pd.read_excel(_replace_path, sheet_name=0).to_dict('records')
# 检查部位词汇清洗
PartReplaceTable = pd.read_excel(
    _replace_path, sheet_name=1).to_dict('records')
ConditionReplaceTable = pd.read_excel(
    _replace_path, sheet_name=2).to_dict('records')
TitleConditionReplaceTable = pd.read_excel(
    _replace_path, sheet_name=3).to_dict('records')
#正常测量值
nomalMeasure = pd.read_excel(os.path.join(_DICTIONARIES_DIR, "报告助手部位词典.xlsx"), sheet_name=2)
...
def Str_replace(Str, title=False):
    ...
    for row in Replace_table:
        ...
        if row['替换值'] is np.nan:
            ...
            Str = Str.upper().replace(row["原始值"], row["替换值"])
            ...
            for m in range(len(sentence)-1):
                ...
                for n in range(m+1, len(sentence)):
                    if (set(sentence[m]['partlist']) >= set(sentence[n]['partlist']) and
                            sentence[m]['orientation'] == sentence[n]['orientation'] and
...
if __name__ == "__main__":
    ...
    ReportStr = """   冠状动脉钙化总积分: 3.70；冠状动脉左主干钙化积分: 0.00, 左前降支钙化积分: 0.00；左回旋支钙化积分: 0.00；右冠状动脉钙化积分: 3.70
    """
    # D1未见斑块
    # """
    StudyPart = """腕关节CT平扫（右侧）;肋骨CT平扫;X线计算机体层(CT)加收(三维重建);X线计算机体层(CT)扫描加收(使用螺旋扫描);"""
